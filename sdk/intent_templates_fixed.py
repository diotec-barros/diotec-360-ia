...
def instantiate_template(intent_name: str, params: Dict[str, Any]) -> Optional[str]:
    """
    Instantiate a template with user parameters - FLAT-LOGIC
    Returns the Aethel code ready for Z3 verification
    
    FLAT-LOGIC Rules:
    - No 'let' declarations
    - Only direct comparisons in solve block
    - Pre-computed values inserted as literals
    """
    template = get_template(intent_name)
...
"""
    ...
    # MULTISIG: Multi-signature authorization
        ...
        signed_count = len(params.get('signed', []))
...
"""
    ...
    # VOTE: One person, one vote
        ...
        has_voted = 1 if params.get('voted_before', False) else 0
...
"""
    ...
    # DELIVERY: GPS-based delivery confirmation
        ...
        import math
        lat_diff = abs(params['gps_lat'] - params['target_lat'])
        lon_diff = abs(params['gps_lon'] - params['target_lon'])
        distance_meters = int(math.sqrt(lat_diff**2 + lon_diff**2) * 111000)
        tolerance = int(params['tolerance_meters'])
        is_within_tolerance = distance_meters <= tolerance
