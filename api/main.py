...
"""
DIOTEC360 API - Backend for DIOTEC360-Studio
...
# Configure CORS
_cors_raw = (getenv("DIOTEC360_CORS_ORIGINS", legacy="DIOTEC360_CORS_ORIGINS", default="") or "").strip()
...
# Include Memory Bridge router (v3.3.0 - Unified Merkle Memory)
app.include_router(memory_router)

# Include Lattice Bridge router (v3.4.0 - Proof Mining)
...
class SovereignAuthEnvelope(BaseModel):
    publicKeyHex: str
    timestamp: int
    nonce: str
    proofHash: str
...
class RpcEnvelope(BaseModel):
    ...
    auth: SovereignAuthEnvelope


_rpc_nonce_cache: Dict[str, int] = {}
_rpc_nonce_lock = asyncio.Lock()
...
async def _nonce_seen_or_mark(nonce: str, now_ms: int, ttl_ms: int) -> bool:
    async with _rpc_nonce_lock:
        expired_before = now_ms - ttl_ms
        if _rpc_nonce_cache:
            for k, v in list(_rpc_nonce_cache.items()):
                if v < expired_before:
                    del _rpc_nonce_cache[k]

        if nonce in _rpc_nonce_cache:
            return True

        _rpc_nonce_cache[nonce] = now_ms
        return False
...
@app.post("/api/rpc")
async def rpc_bridge(
    envelope: RpcEnvelope,
    request: Request,
    x_diotec360_pubkey: Optional[str] = Header(default=None),
    x_diotec360_proofhash: Optional[str] = Header(default=None),
    x_diotec360_signature: Optional[str] = Header(default=None),
    x_diotec360_timestamp: Optional[str] = Header(default=None),
    x_diotec360_nonce: Optional[str] = Header(default=None),
):
    try:
        try:
        ...
        if not DIOTEC360Crypt:
            return {
                ...
                "error": {"code": -32000, "message": "Crypto engine unavailable"},
            }

        auth = envelope.auth
        pub = (x_diotec360_pubkey or auth.publicKeyHex or "").strip()
        proof_hash = (x_diotec360_proofhash or auth.proofHash or "").strip()
        sig = (x_diotec360_signature or auth.signatureHex or "").strip()
        nonce = (x_diotec360_nonce or auth.nonce or "").strip()

        if x_diotec360_timestamp is not None:
            try:
                ts_ms = int(x_diotec360_timestamp)
            except Exception:
                ts_ms = int(auth.timestamp)
        else:
            ts_ms = int(auth.timestamp)

        if not pub or not proof_hash or not sig or not nonce or not ts_ms:
            return {
                "jsonrpc": "2.0",
                "id": envelope.request.id,
                "error": {"code": -32001, "message": "Missing sovereign auth fields"},
            }

        now_ms = int(time.time() * 1000)
        if abs(now_ms - ts_ms) > 5000:
            return {
                "jsonrpc": "2.0",
                "id": envelope.request.id,
                "error": {"code": -32002, "message": "Timestamp outside tolerance"},
            }

        if await _nonce_seen_or_mark(nonce, now_ms, ttl_ms=120000):
            return {
                "jsonrpc": "2.0",
                "id": envelope.request.id,
                "error": {"code": -32003, "message": "Replay detected (nonce reused)"},
            }

        canonical = _canonical_json(
            {
                "payload": envelope.request.model_dump(),
                "timestamp": ts_ms,
                "nonce": nonce,
                "publicKeyHex": pub,
            }
        )
        expected_proof_hash = _sha256_hex(canonical)
        if expected_proof_hash != proof_hash:
            return {
                "jsonrpc": "2.0",
                "id": envelope.request.id,
                "error": {"code": -32004, "message": "Proof hash mismatch"},
            }

        crypt = DIOTEC360Crypt()
        if not crypt.verify_signature(pub, proof_hash, sig):
            return {
                "jsonrpc": "2.0",
                "id": envelope.request.id,
                "error": {"code": -32005, "message": "Invalid signature"},
            }

        if envelope.request.jsonrpc != "2.0":
            return {
                "jsonrpc": "2.0",
                "id": envelope.request.id,
        ...
        if envelope.request.method == "diotec360.verify":
            code = (envelope.request.params or {}).get("code")
            if not isinstance(code, str):
                return {
                    "jsonrpc": "2.0",
                    "id": envelope.request.id,
                    "error": {"code": -32602, "message": "Missing param: code"},
                }
            ...
            if not intent_map:
                return {
                    ...
                    "result": {
                        ...
                        "badge": "unverified",
                        "status": "PARSE_ERROR",
                        "message": "Failed to parse DIOTEC360 code",
                        "details": {"errors": ["Invalid syntax"], "intents": []},
                    },
                }
            ...
            for intent_name in intent_map.keys():
                try:
                    r = judge.verify_logic(intent_name)
                    status = r.get("status", "ERROR") if isinstance(r, dict) else "ERROR"
                    message = r.get("message", "Unknown error") if isinstance(r, dict) else "Unknown error"
                    results.append({"name": intent_name, "status": status, "message": message})
                    if status != "PROVED":
                        all_proved = False
                except Exception as e:
                    results.append({"name": intent_name, "status": "ERROR", "message": str(e)})
                    all_proved = False

            badge = "certified" if all_proved else "unverified"
        ...
        return {
            "jsonrpc": "2.0",
            "id": envelope.request.id,
            "error": {"code": -32601, "message": f"Method not found: {envelope.request.method}"},
        }

    except Exception as e:
        return {
            "jsonrpc": "2.0",
            "id": envelope.request.id if envelope and envelope.request else "0",
...
def _parse_lattice_nodes() -> List[str]:
    raw = (getenv("DIOTEC360_LATTICE_NODES", legacy="DIOTEC360_LATTICE_NODES", default="") or "").strip()
    if not raw:
        return []
    nodes = []
...
async def lattice_state():
    ...
    privkey_hex = getenv("DIOTEC360_NODE_PRIVKEY_HEX", legacy="DIOTEC360_NODE_PRIVKEY_HEX")
    if privkey_hex and DIOTEC360Crypt:
        try:
...
async def _lattice_startup() -> None:
    ...
    print("\n" + "="*70)
    print("[SHIELD] DIOTEC360 LATTICE v3.0.3 - HYBRID SYNC PROTOCOL")
    print("="*70)
...
async def _http_sync_heartbeat():
    ...
    """
    lattice_nodes = (getenv("DIOTEC360_LATTICE_NODES", legacy="DIOTEC360_LATTICE_NODES", default="") or "").strip()
...
async def _force_state_reconciliation(client: httpx.AsyncClient, peer_url: str, peer_root: str):
    ...
    try:
        ...
        if peer_full_state.get("signed", False) and DIOTEC360Crypt:
            ...
            trusted_pubkeys = (getenv("DIOTEC360_TRUSTED_STATE_PUBKEYS", legacy="DIOTEC360_TRUSTED_STATE_PUBKEYS", default="") or "").strip()
            if trusted_pubkeys:
...
async def verify_code(request: VerifyRequest):
    """
    Verify DIOTEC360 code using the Judge (Z3 Solver)
    """
    try:
...
async def compile_code(request: CompileRequest):
    """
    Compile DIOTEC360 code (verify + generate implementation)
    """
    try:
        # First verify
...
async def execute_code(request: ExecuteRequest):
    """
    Execute DIOTEC360 code in WASM runtime
    """
    try:
...
async def paypal_webhook(request: Request):
    ...
    - PAYMENT.SALE.COMPLETED: Payment completed and funds captured
    
    Security: Validates PayPal signature to prevent fraud
    """
    try:
        ...
        logger.info(f"PayPal webhook received: {headers.get('paypal-transmission-id', 'unknown')}")
        ...
        if event_type == "CHECKOUT.ORDER.APPROVED":
            # Order approved - log for tracking
            order_id = resource.get("id", "unknown")
            logger.info(f"Order approved: {order_id}")
            
            return {
                "success": True,
                "message": "Order approval recorded",
                "event_type": event_type,
                "order_id": order_id
            }
            
        elif event_type == "PAYMENT.SALE.COMPLETED":
            # Payment completed - add credits to user account
            sale_id = resource.get("id", "unknown")
            amount = resource.get("amount", {}).get("total", "0")
            currency = resource.get("amount", {}).get("currency", "USD")
            payer_email = resource.get("payer", {}).get("email_address", "unknown")
            
            logger.info(f"Payment completed: {sale_id} - {amount} {currency} from {payer_email}")
